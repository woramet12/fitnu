...
export default function VerifyEmail() {
  ...
  // ดึง email จาก query หรือจาก currentUser เผื่อไม่มี query
  ...
  const handleResend = async () => {
    ...
    setBusy(true);
    try {
      // Reload user เผื่อเพิ่งยืนยันไปแล้ว
      ...
      setCooldown(30); // กันกดรัว 30 วินาที
    } catch (err) {
      const code = err?.code || "";
      let msg = "ส่งอีเมลยืนยันไม่สำเร็จ";
      if (code === "auth/too-many-requests")
        msg = "ส่งคำขอถี่เกินไป โปรดลองใหม่อีกครั้งภายหลัง";
